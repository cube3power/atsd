...
The method accepts `GET` and `POST` requests with optional JSON payload, flattens the request parameters and the payload into a collection of key/values which are mapped to message fields. The derived message is then stored in the database and processed in the rule engine.

A set of reserved parameter names, specified in the query string, are provided to control the mappings and to discard unnecessary data.

The method can be used to process HTTP notifications generated by external web services such as GitHub, AWS SNS, Jenkins, Slack and Telegram Bots etc.
...
JSON fields of primitive types (string, number, boolean) are converted into message **tags**, where tag name equals the field's full name (contains parent object names, separated by `.` character) and tag value equals field value.
...
1. By default, the entity is set to the remained of the path following the `/api/v1/messages/webhook/` prefix.
...
2. The entity can be specified literally by adding an `entity` parameter to the query string, for example `/api/v1/messages/webhook/jenkins?entity=test-1`
...
* Message `entity` is set to the remainder of the URL path after the `/webhook/` part (and before the query string). If the remainder is empty, the `entity` must be specified as described in the 'Entity Mapping' section above.
