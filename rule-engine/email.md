...
The email action enables delivery of messages to one or more email subscribers based on [window](window.md) status events.

Notifications can be triggered when the window changes its status as well as at a scheduled interval while its status is `REPEAT`.

## Email Client

Messages generated by the rule engine are handled by the the built-in [mail client](../administration/setting-up-email-client.md) which is accessible on the **Settings > Mail Client** page.

Client settings apply to all outgoing messages.
...
| Recipients | One or multiple email subscribers who shall receive a scheduled message. Use a comma or semi-colon to separate multiple addresses. |
...
| Subject | Custom subject text for each status separately. A list of supported placeholders is provided below. In addition to built-in placeholders you can use expressions containing built-in functions, for example: `${round(threshold_linear_time(99))/60}` or `${round(avg())}` |
...
A header and footer can be specified in both plain text and HTML format in the [email client](../administration/setting-up-email-client.md) settings and applied to all messages. 
