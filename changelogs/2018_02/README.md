...
5041 | rule engine | Bug | Telegram doesn't support `<br>` when parse_mode is `html`.
...
5030 | rule engine | Bug | Fix Telegram notification timeouts.
...
5021 | sql | Bug | `LAST(datetime)` displays UNIX time instead of datetime.
...
5013 | rule engine | Bug | System command execution error should be registered in Rule Errors.
...
5005 | entity_views | Bug | Add validation and display error message at runtime.
5004 | rule engine | Bug | Default values for web notification parameters are ignored.
5002 | rule engine | Bug | Disable notifications for related messages in Telegram.
...
4981 | sql | Feature | Implemenent `HTML` format in scheduled SQL reports.
4976 | data-in | Support | [tcollector](../../integration/tcollector#tcollector) installation update.
4973 | rule editor | Bug | Append `[Disabled]` suffix to web configuration name if it had been disabled.
...
Issue| Category    | Type    | Subject              
-----|-------------|---------|----------------------
5060 | json | Bug | NPE in Test.
5052 | docker | Bug | `docker.fs.total.size.rw` sampling frequency irregularity.
...
4997 | admin | Support | Set `XmX` in collector containers launched with Docker job to 256 megabytes.
