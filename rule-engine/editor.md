...
| Condition | [Condition](condition.md) is a boolean expression which is evaluated each time a data sample is received by the window. For example, the condition `value > 50` checks if a received value is greater than 50. If the expression evaluates to `true`, it raises an alert, followed by an execution of triggers such as a system command or email notification. Once the expression returns 'false', the alert is closed and another set of triggers is invoked. The expression consists of one or multiple checks combined with `OR` and `AND` operators. Exceptions specified in the Override table take precedence over the default condition.
| Variables | List of custom fields with aliases which can be be accessed in placeholders by assigned name. |
